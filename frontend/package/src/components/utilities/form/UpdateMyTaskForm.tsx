...
// Formats an ISO date string as "1st April, 2025" with correct ordinal suffixes.
...
// Parses a DB date string into a local Date without UTC offset shifting the day.
const parseApiDate = (raw: string): Date => {
  const [y, m, d] = raw.split('T')[0].split('-').map(Number);
  return new Date(y, m - 1, d);
};

// Serialises a Date to the "YYYY-MM-DD" format expected by the backend API.
...
// Allows a logged-in user to update their own tasks. Tasks they created expose all editable fields;
// tasks assigned by someone else show only the status selector.
const UpdateMyTaskForm = () => {
  ...
  // Populates edit fields when a task is selected; owned tasks get full field bindings,
  // assigned tasks only bind the status field.
  ...
  // Skips the API call when nothing has changed; otherwise PATCHes the task and
  // updates local state to keep the UI in sync without a full refetch.
