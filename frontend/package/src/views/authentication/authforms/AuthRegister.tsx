...
// Lightweight email format check — trims leading/trailing whitespace before testing.
const isValidEmail = (v: string) => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(v.trim());

// Registration form with per-field lazy validation and a full re-touch on submit
// to surface all errors at once if the user clicks Register without filling fields.
const AuthRegister = () => {
  ...
  // Flags a field as interacted with so validation errors become visible for that field.
  ...
  // Submits registration data to the backend; on success redirects to login.
  // Server-side duplicate-email check (409) surfaces as a user-facing error message.
