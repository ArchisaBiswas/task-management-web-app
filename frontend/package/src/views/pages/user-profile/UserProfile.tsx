...
// User profile page. Opens a modal for editing; changes are staged in temp state
// so the displayed values are not mutated until the user explicitly saves.
const UserProfile = () => {
    ...
    // Re-syncs temp state each time the modal opens so edits from a previous cancelled
    // session don't bleed into the next one.
    useEffect(() => {
    ...
    // Persists personal info to the backend and updates both local state and the auth context
    // so the header/profile card reflect the new name and timezone immediately.
